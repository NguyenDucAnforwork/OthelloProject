...
class ComputerPlayer:
    ...
    def computerHard1(self, grid, depth, alpha, beta, player, numMove):
        ...
        if player < 0:   # minimizing player
            ...
            for move in availMoves:
                ...
                bMove, value = self.computerHard1(newGrid, depth-1, alpha, beta, player *-1, numMove+1)
        ...
        if player > 0:    # maximizer
            ...
            for move in availMoves:
                ...
                bMove, value = self.computerHard1(newGrid, depth-1, alpha, beta, player*-1, numMove+1)   # it's the best move of the opponent
    ...
    def computerHard2(self, grid, depth, alpha, beta, player, numMove):
        newGrid = copy.deepcopy(grid)
        availMoves = self.grid.findAvailMoves(newGrid, player)
        # if there's no feasible move and we're still not in the leaf node yet
        if numMove == 64 or depth == 0 or len(availMoves) == 0:
        ...
        if player < 0:   # minimizing player
            ...
            for move in availMoves:
                ...
                # we need to update the grid before calling recursive function because we evaluate positions in the future
                bMove, value = self.computerHard2(newGrid, depth-1, alpha, beta, player *-1, numMove+1)
        ...
        if player > 0:    # maximizer
            ...
            for move in availMoves:
                ...
                bMove, value = self.computerHard2(newGrid, depth-1, alpha, beta, player*-1, numMove+1)   # it's the best move of the opponent
